...
void	ConfigServer::checkPort(string & block){
	...
	if (_port < 1024 || _port > 65535)
		throw runtime_error("By convention port must be between 1024 et 65535");
}
...
void	ConfigServer::setRoot(string const & root){
	_root = root;
}

void	ConfigServer::setHost(string const & host){
	_host = host;
}

void	ConfigServer::setMaxBodySize(int maxBodySize){
	_maxBodySize = maxBodySize;}

void	ConfigServer::setAllowedMethod(vector<enum HttpMethod> const & allowedMethod){
	_allowedMethod = allowedMethod;
}

void	ConfigServer::setDirectoryListing(bool directoryListing){
	_directoryListing = directoryListing;
}

void	ConfigServer::setName(string const & name){
	_name = name;
}

void	ConfigServer::setIndex(string const & index){
	_index = index;
}

void	ConfigServer::setErrorPages(map<int,string> const & err){
	_errorPage = err;
}

void	ConfigServer::setReturnURI(map<int,string> const & uri){
	_returnURI = uri;
}

void	ConfigServer::setUploadPath(string const & path){
	_uploadPath = path;
}

void	ConfigServer::setLocation(vector<string> loc){
	_location = loc;
}

void	ConfigServer::setLocationBlock(vector<Location> const & locationBlock){
	_locationBlock = locationBlock;
}

void	ConfigServer::setBinPath(vector<string> const & binPath){
	_binPath = binPath;
}

void	ConfigServer::setCgiExtension(vector<string> const & cgiExtension){
	_cgiExtension = cgiExtension;
}

int	ConfigServer::getServerIndex(void) const {
	return(_indexServer);
}

string const &	ConfigServer::getServerBlock(void) const {
	return(_serverBlock);
}

int	ConfigServer::getPort(void) const {
	return(_port);
}

string const &	ConfigServer::getRoot(void) const {
	return(_root);
}

string const &	ConfigServer::getHost(void) const {
	return(_host);
}

int	ConfigServer::getMaxBodySize(void) const {
	return(_maxBodySize);
}

vector<enum HttpMethod> const &	ConfigServer::getAllowedMethod(void) const {
	return(_allowedMethod);
}

bool	ConfigServer::getDirectoryListing(void) const {
	return(_directoryListing);
}

string const &	ConfigServer::getName(void) const {
	return(_name);
}

string const &	ConfigServer::getIndex(void) const {
	return(_index);
}

map<int,string> const &	ConfigServer::getErrorPages(void) const {
	return(_errorPage);
}

map<int,string> const &	ConfigServer::getReturnURI(void) const {
	return(_returnURI);
}

string const &	ConfigServer::getUploadPath(void) const {
	return(_uploadPath);
}

vector<string> const &	ConfigServer::getLocation(void) const {
	return(_location);
}

vector<Location> const &	ConfigServer::getLocationBlock(void) const {
	return(_locationBlock);
}

vector<string> const &	ConfigServer::getBinPath(void) const {
	return(_binPath);
}

vector<string> const &	ConfigServer::getCgiExtension( void ) const {
