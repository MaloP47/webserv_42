...
/*   Updated: 2024/04/13 11:15:53 by gbrunet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static std::string	fileSize(off_t size) {
...
static std::string	htmlDir(struct dirent *dp, std::string path) { // need to edit the href...
	char		date[128];
	struct tm	*info;
	size_t		length;
	struct stat	result;
	...
	if (stat((path + dp->d_name).c_str(), &result) == 0) {
		info = std::localtime(&result.st_ctime);
...
static std::string	htmlFile(struct dirent *dp, std::string path) {	
	char				date[128];
	struct tm			*info;
	size_t				length;
	struct stat			result;
	...
	std::string	html = "<tr><th><svg xmlns=\"http://www.w3.org/2000/svg\" \
						...
						color:#8585f2\" href=\"" + path + dp->d_name + "\"> ";
...
std::string	DirectoryListing::html(std::string path) {
	...
	struct dirent	*dp;
	std::string		html;
	std::map<std::string, struct dirent*>	folder;
	std::map<std::string, struct dirent*>	file;
	...
	while ((dp = readdir(dir)) != NULL) {
		if (dp->d_type == DT_DIR)
			folder.insert(std::pair<std::string, struct dirent*>(dp->d_name, dp));
//			html += htmlDir(dp);
		else
			file.insert(std::pair<std::string, struct dirent*>(dp->d_name, dp));
//			html += htmlFile(dp);
	}
	for (std::map<std::string, struct dirent*>::iterator it = folder.begin(); it != folder.end(); it++)
		html += htmlDir(it->second, path);
	for (std::map<std::string, struct dirent*>::iterator it = file.begin(); it != file.end(); it++)
		html += htmlFile(it->second, path);
